/**
 * Pure brand colour helpers — safe for client components.
 *
 * Do not import `./content` here; that pulls `node:fs` into the browser bundle
 * and Turbopack panics on App Router client chunks.
 */
...
export const BRAND_ACCENT_FALLBACK: BrandAccent = {
...
export function accentFromColor(color: string): BrandAccent {
  ...
  if (!rgb) return BRAND_ACCENT_FALLBACK;
...
/**
 * Hex palette for a grainient backdrop from an optional issue colour.
 * When `color` is omitted, falls back to the site's default blue hue.
 */
export function grainientPalette(color?: string): GrainientPalette {
  const rgb = color ? parseRgb(color) : null;
...
/**
 * Readable CSS colour for issue-scoped UI (hovers, borders, primary button).
 *
 * Never pass the raw low-alpha `themeColor` into text styles — those values
 * are for cover glows and read as muddy dark reds/blues on hover.
 */
export function issueAccentValue(themeColor: string): string {
 ...
 *
 * `--issue-accent` matches `--primary` / the PDF button.
 */
export function issueAccentVars(themeColor: string): Record<string, string> {
  const accent = accentFromColor(themeColor);
  const readable = issueAccentValue(themeColor);
  return {
    "--issue-accent": readable,
    "--accent": readable,
